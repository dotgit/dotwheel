...
class Http
{
    ...
    /** parse xml document into an array
    ...
    public static function xml2Array($xml)
    {
        ...
        xml_parser_set_option($parser, XML_OPTION_TARGET_ENCODING, Nls::$charset);
        xml_parser_set_option($parser, XML_OPTION_CASE_FOLDING, 0);
        xml_parser_set_option($parser, XML_OPTION_SKIP_WHITE, 1);
        if (! xml_parse_into_struct($parser, $xml, $pieces))
            return array();
        ...
        foreach ($pieces as $piece)
        {
            ...
            {
            case 'complete':
                $value = isset($piece['value']) ? $piece['value'] : null;
                if (isset($elements[$tag]))
                {
                    if (is_array($elements[$tag]))
                        $elements[$tag][] = $value;
                    else
                        $elements[$tag] = array($elements[$tag], $value);
                }
                else
                    $elements[$tag] = $value;
                break;
            case 'open':
                if (isset($elements[$tag]))
                {
                    if (isset($elements[$tag][0]))
                        $elements[$tag][] = array();
                    else
                        $elements[$tag] = array($elements[$tag], array());
                }
                else
                    $elements[$tag] = array();
                $stack[$stack_pos++] = &$elements;
                if ($cnt = count($elements[$tag]))
                    $elements = &$elements[$tag][$cnt - 1];
                else
                    $elements = &$elements[$tag];
                break;
            case 'close':
                $elements = &$stack[--$stack_pos];
                break;
            }
        }
