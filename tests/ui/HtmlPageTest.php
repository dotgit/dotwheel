...
/**
 * Generated by PHPUnit_SkeletonGenerator on 2016-04-26 at 10:06:04.
 * @coversDefaultClass Dotwheel\Ui\HtmlPage
 * @requires PHP 5.4
 */
class HtmlPageTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testGetHead()
    {
        ...
        $this->assertContains('<meta name="viewport" content="viewport content">', $res, 'initial meta in place');
        $this->assertContains('<meta charset="utf-8">', $res, 'another meta added');
        $this->assertLessThan(
            strpos($res, '<meta charset="utf-8">'),
            strpos($res, '<meta name="viewport"'),
            'original header before subsequent header'
        );
        $this->assertContains('<title>&lt;page&gt; &quot;overwrite&quot; title</title>', $res, 'second title overwrites');
        $this->assertLessThan(
            strpos($res, '<title>&lt;page&gt;'),
            strpos($res, '<meta name="viewport"'),
            'viewport meta before title'
        );
        $this->assertNotContains('<meta name="description"', $res, 'second description overwrites or deletes');
        $this->assertContains('<link href="favicon.ico" rel="shortcut icon" type="image/x-icon">', $res);
        $this->assertLessThan(
            strpos($res, '<link href="favicon.ico"'),
            strpos($res, '<title>&lt;page&gt;'),
            'title before favicon'
        );
        $this->assertContains('<base target="_blank">', $res);
        $this->assertLessThan(
        ...
        $this->assertContains('<link rel="stylesheet" href="&quot;style&quot;.css">', $res);
        $this->assertLessThan(
            strpos($res, '<link rel="stylesheet"'),
            strpos($res, '<meta charset="utf-8">'),
            'charset before stylesheet'
        );
        $this->assertContains('<style>.important{color:red;}</style>', $res);
        $this->assertLessThan(
    ...
    public function testGetTail()
    {
        ...
        // html footer
        $this->assertContains('html footer', $res, 'original footer present');
        $this->assertContains('html another footer', $res, 'another footer appended');
        $this->assertLessThan(
            strpos($res, 'html another footer'),
            strpos($res, 'html footer'),
            'original footer before subsequent footer'
        );
        $this->assertNotContains('html duplicate footer', $res, 'other duplicates do not overwrite');
        $this->assertContains('<script src="&quot;initial&quot;.js"></script>', $res, '"initial".js present');
        $this->assertLessThan(
            strpos($res, '&quot;initial&quot;.js'),
            strpos($res, 'html another footer'),
            'html footer before initial js file'
        );
        $this->assertContains('<script src="&quot;another&quot;.js"></script>', $res, '"another".js present');
        $this->assertLessThan(
        ...
        $js = $m[1];
        $this->assertContains('var a=true;', $js, 'js code present');
        // DOM ready block
        $this->assertEquals(
            1,
        ...
        $dr = $n[1];
        $this->assertContains('a++;', $dr, 'initial DOM ready code in place');
        $this->assertContains('anotherCode();', $dr, 'additional DOM ready code in place');
        $this->assertLessThan(
        ...
        // js block continued
        $this->assertContains('lastCommand();', $js, 'last js code present');
        $this->assertLessThan(
        ...
        // last html footer
        $this->assertContains('html last footer', $res, 'last footer appended');
        $this->assertLessThan(
