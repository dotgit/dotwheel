...
class Paginator
{
    public const P_ACTIVE = 1;
    public const P_RANGE = 2;
    public const P_LAST = 3;
    public const P_MAX_SLOTS = 4;
     ...
     */
    public static function getListBinary(array $params): array
    {
     ...
     * compatibility call:<pre>
     *  getListLinear($page+1, (int)(($total_items-1) / $items_per_page) + 1, $max_slots=10, $items=100)</pre>
     *
     * @param array $params {P_ACTIVE:current page number,
     *  P_LAST: last page number,
     *  P_SLOTS: how many pages to display
     ...
     * @return array an array of page numbers to display
     */
    public static function getListLinear(array $params): array
    {
        $active_page = Params::extract($params, self::P_ACTIVE);
        $last_page = Params::extract($params, self::P_LAST);
    ...
    /** use inverse logarithmic scale for big page sets
     *
     * @param array $params {P_ACTIVE:current page number,
     *  P_LAST: last page number,
     *  P_SLOTS: how many pages to display
     ...
     * @return array an array of page numbers to display
     */
    public static function getListLog(array $params): array
    {
        $active_page = Params::extract($params, self::P_ACTIVE);
        $last_page = Params::extract($params, self::P_LAST);
        $slots = Params::extract($params, self::P_MAX_SLOTS, 10);
        ...
        if ($r) {
            for (
                $d = ($active_page + 1 + $last_page) >> 1, $i = $slots - 2, $j = $slots - $r - 2;
